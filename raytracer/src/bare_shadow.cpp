	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			if (found_obj) {
				...
				// generate shadow ray or secondary ray just by diplacement between
				// light location and hit point
				ray shadow_ray(hit_point, light - hit_point);

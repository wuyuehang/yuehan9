...
int main()
{
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			// 16x order sample pattern
			vector<vec3> subpixels;
			subpixels.push_back(current_pixel + vec3(-1./6/W, +1./6/H, 0));
			subpixels.push_back(current_pixel + vec3(-2./6/W, +1./6/H, 0));
			subpixels.push_back(current_pixel + vec3(-2./6/W, +2./6/H, 0));
			subpixels.push_back(current_pixel + vec3(-1./6/W, +2./6/H, 0));
			...
			quad[i*W*3+j*3] = static_cast<unsigned char>(rgb[0] >> 4);
			quad[i*W*3+j*3+1] = static_cast<unsigned char>(rgb[1] >> 4);
			quad[i*W*3+j*3+2] = static_cast<unsigned char>(rgb[2] >> 4);
		}
	}

	dumper.serialize("16xaa_simplest_ortho_raycast.ppm", W, H, quad);

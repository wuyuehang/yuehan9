...
void compile_shader(void)
{
	...
	g_rotate = glGetUniformLocation(g_program, "rotate");
	assert(g_rotate != 0xFFFFFFFF);
}
...
void render(void)
{
	glm::mat4 rotate_mat = glm::rotate(glm::radians(45.0f), glm::vec3(1.0f, 1.0f, 1.0f));

	glUniformMatrix4fv(g_rotate, 1, GL_FALSE, &rotate_mat[0][0]);

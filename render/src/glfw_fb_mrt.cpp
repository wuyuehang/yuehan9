...
int main(int argc, char **argv)
{
	...
	GLuint vao_left_pass21;
	glGenVertexArrays(1, &vao_left_pass21);
	glBindVertexArray(vao_left_pass21);
	...
	load_shader_from_file("shaders/glfw_fb_mrt_stage1.vert", &source);
	...
	free(source);
	load_shader_from_file("shaders/glfw_fb_mrt_stage1.frag", &source);
	...
	GLuint pos_texture_pass1;
	glGenTextures(1, &pos_texture_pass1);
	glBindTexture(GL_TEXTURE_2D, pos_texture_pass1);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 500, 500, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, pos_texture_pass1, 0);
	assert(GL_FRAMEBUFFER_COMPLETE == glCheckFramebufferStatus(GL_FRAMEBUFFER));
	...
	/* for left image */
	GLuint vao_left_pass2;
	glGenVertexArrays(1, &vao_left_pass2);
	glBindVertexArray(vao_left_pass2);

	GLuint vbo_pos_left_pass2;
	glGenBuffers(1, &vbo_pos_left_pass2);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_pos_left_pass2);
	glBufferData(GL_ARRAY_BUFFER, sizeof(left_img_pass2), left_img_pass2, GL_STATIC_DRAW);
	...
	/* for right image */
	GLuint vao_right_pass2;
	glGenVertexArrays(1, &vao_right_pass2);
	glBindVertexArray(vao_right_pass2);

	GLuint vbo_pos_right_pass2;
	glGenBuffers(1, &vbo_pos_right_pass2);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_pos_right_pass2);
	glBufferData(GL_ARRAY_BUFFER, sizeof(right_img_pass2), right_img_pass2, GL_STATIC_DRAW);
	...
	glDeleteTextures(1, &pos_texture_pass1);
	glDeleteFramebuffers(1, &fbo);

	glDeleteBuffers(1, &vbo_pos_pass1);
	glDeleteBuffers(1, &vbo_inst_pass1);
	glDeleteVertexArrays(1, &vao_left_pass21);

	glDeleteBuffers(1, &vbo_pos_left_pass2);
	glDeleteBuffers(1, &vbo_pos_right_pass2);
	glDeleteBuffers(1, &vbo_texcord_pass2);
	glDeleteBuffers(1, &vbo_index_pass2);
	glDeleteVertexArrays(1, &vao_left_pass2);
	glDeleteVertexArrays(1, &vao_right_pass2);
	glfwTerminate();

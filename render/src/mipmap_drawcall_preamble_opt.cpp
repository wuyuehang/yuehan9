...
#define _mipmapchain_need_depth	0
...
#if _mipmapchain_need_depth
	GLuint dstDepthTexObj;
	glGenTextures(1, &dstDepthTexObj);
	glBindTexture(GL_TEXTURE_2D, dstDepthTexObj);
	glTexImage2D(GL_TEXTURE_2D,
			0, // base level
...
#if _mipmapchain_need_depth
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, dstDepthTexObj, 0);
	assert(GL_FRAMEBUFFER_COMPLETE == glCheckFramebufferStatus(GL_FRAMEBUFFER));
#endif

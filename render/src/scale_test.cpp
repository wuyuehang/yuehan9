...
const GLchar* vs_src = "#version 330 core\n"
		...
		"void main()"
		"{"
		"gl_Position = scale * vec4(position.x, position.y, position.z, 1.0);"
		"}";
...
void compile_shader(void)
{
	...
	g_scale = glGetUniformLocation(g_program, "scale");
	assert(g_scale != 0xFFFFFFFF);
}
...
void render(void)
{
	/* scale target triangle in +0.5x, +0.5y, +0.5z */
	glm::mat4 scale_mat = glm::scale(glm::vec3(0.5f, 0.5f, 0.5f));

	glUniformMatrix4fv(g_scale, 1, GL_FALSE, &scale_mat[0][0]);
